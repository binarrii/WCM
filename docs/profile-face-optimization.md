...
因此长视频的额外人脸调用目标上限约为 30%；短视频为了让优化能够生效，向上取整可能产生 1 次补帧。补帧并发默认为 4，可设为 ≤ 0 的整数取消补帧并发限制，补帧总量上限仍有效。视频查询移除了逐个读取图库样本框的额外请求。日志会记录主/复用人脸调用数、补帧数、确认数和疑似数。
...
| `face_neighbor_concurrency` | `4` | 每个视频的补帧识别并发；正数范围 1～8，≤ 0 不限制。集群 InsightFace 并发另由 `insightface_concurrency` 控制 |
