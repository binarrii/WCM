...
```dotenv
WCM_AUTH_TRUSTED_PROXIES=["172.25.0.0/16","10.252.25.198/32"]
```
...
登录和注册页保留四套 SVG 背景：深蓝光学、银白玻璃、青绿扫描、靛紫流光。
默认在进入页面时随机选择一次，切换登录 / 注册表单不会重新抽取。
右上角“背景”可选定具体方案并固定到当前浏览器，选择“每次随机”会清除固定偏好。
偏好使用同源 `localStorage` 的 `wcm.auth.background.v1`，仅保存受支持的背景标识；
无效值回退随机，存储不可用时仍可切换背景并提示无法保存，不影响账户操作。
背景配色仅作用于登录页，不修改工作台主题。背景为装饰图片，不包含身份数据。

本次背景更新只需构建和更新 WebUI，保留 API、Worker 与人物识别服务实例。
...
### 2026-09-16 Passkey 信息验证

- 后端认证、Passkey 信息与头像测试 100 项通过，前端 115 项通过，Ruff 与生产构建通过。
- 桌面和 390 px 手机视口确认管理器名称、秒级时间、IPv4/IPv6、历史记录提示正常显示。
