...
const fetchRecords = async (reset = false) => {
  ...
  try {
    const response = await axios.get(`${API_BASE}/face_records`, {
      params: {
        page: page.value,
        limit: limit.value,
...
const fetchStats = async () => {
  try {
    const response = await axios.get(`${API_BASE}/face_records/stats`);
    const data = response.data;
    stats.value = {
      total: data.total,
...
// Filtered records (already filtered by backend, so just return the accumulated records)
...
const handleSubmit = async () => {
  ...
  try {
    if (isEditMode.value) {
      await axios.put(`${API_BASE}/face_records/${editingRecordId.value}`, {
      ...
      await axios.post(`${API_BASE}/face_records`, formData, {
        headers: {
...
const handleDelete = async (record) => {
  ...
  try {
    await axios.delete(`${API_BASE}/face_records/${record.id}`);
    showToast('记录删除成功');
    fetchRecords(true);
    fetchStats();
  } catch (error) {
...
const handleScroll = () => {
  ...
  // Trigger when scrolled to 150px from bottom
